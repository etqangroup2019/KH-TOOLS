...
def set_glare_type(glare_node, glare_type_value):
    ...
    if bpy.app.version >= (5, 0, 0):
        ...
        # Use a mapping to ensure correct uppercase enum names
        glare_type_map = {
            'Bloom': 'BLOOM',
            'Ghosts': 'GHOSTS',
            'Streaks': 'STREAKS',
            ...
            'Sun Beams': 'STREAKS', # Fallback for 4.x
            'Kernel': 'FOG_GLOW'     # Fallback for 4.x
        }
        mapped_value = glare_type_map.get(glare_type_value, str(glare_type_value).upper())
        
        # Validate against enum items to avoid TypeError
        if hasattr(glare_node, 'glare_type'):
            valid_items = [item.identifier for item in glare_node.bl_rna.properties['glare_type'].enum_items]
            if mapped_value in valid_items:
                glare_node.glare_type = mapped_value
            elif 'FOG_GLOW' in valid_items:
...
def set_glare_quality(glare_node, quality_value):
    ...
    if bpy.app.version >= (5, 0, 0):
        ...
        if hasattr(glare_node, 'quality'):
            ...
            mapped_value = quality_map.get(quality_value, str(quality_value).upper())
            try:
                glare_node.quality = mapped_value
            except:
                pass
...
def add_nod(tree):
    ...
    # 3. Create Glare types based on Blender version
    if bpy.app.version >= (5, 0, 0):
        glare_types = ['Bloom', 'Ghosts', 'Streaks', 'Fog Glow', 'Simple Star', 'Sun Beams', 'Kernel']
    else:
        # Check what types are actually supported in this 4.x version
        temp_node = nodes.new('CompositorNodeGlare')
        valid_items = [item.identifier for item in temp_node.bl_rna.properties['glare_type'].enum_items]
        nodes.remove(temp_node)
        
        all_types = ['Bloom', 'Ghosts', 'Streaks', 'Fog Glow', 'Simple Star']
        name_map = {'Bloom': 'BLOOM', 'Ghosts': 'GHOSTS', 'Streaks': 'STREAKS', 'Fog Glow': 'FOG_GLOW', 'Simple Star': 'SIMPLE_STAR'}
        glare_types = [t for t in all_types if name_map.get(t) in valid_items]
    ...
    for i, g_type in enumerate(glare_types):
        ...
        # Link in chain
        # In 4.x, the input name is usually 'Image'
        target_socket = g_node.inputs[0] # Usually 'Image' or index 0
